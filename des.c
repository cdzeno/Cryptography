...
uint8_t* diff_crypto (L0R0 p1, L0R0 p2, L0R0 c1, L0R0 c2){
  ...
  for (int i = 0; i < index1; i++) {
    for (int j = 0; j < index2; j++) {
      ...
      printf("LastKey[%d]: 0x%x\n", index3, LastKey[index3]);
      index3++;
    }
  }
...
uint8_t** gen_key (L0R0* lista){
  // length Lista = 8
  uint8_t** list_key = malloc(sizeof(uint8_t*)*56);
  int index = 0;

  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if(i != j){
          list_key[index] = diff_crypto(lista[i], lista[j], des(lista[i], key, 3), des(lista[j], key, 3));
          index++;
      }
    }
  }
